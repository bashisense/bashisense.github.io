...
> ***如平台无需求确认设备是否为我司产品，可以忽略此命令的签名，直接返回secret_code用于后续签名即可***
...
```json
{
    ...
    "body":{
        "secret_code":"xxxxxx",       // 签名码，字符串
    }
}
```

> 设备仅在第一次登陆平台时获取secret_code，并存储此值，直到设备被复位到出厂设备，后续一直使用此值签名；
...
> 设备签名，sha1(sort(device-id、secret_code、timestamp、nonce, "register"))。sort的含义是将参数值按照字母字典排序，然后从小到大拼接成一个字符串。本接口计算签名时，需要把字符串常量 “register”参与到计算之中，区别其他场景的签名。
